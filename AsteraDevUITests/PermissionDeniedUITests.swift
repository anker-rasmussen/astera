...
class PermissionUITestCase: AsteraUITestCase {
    ...
    /// Flips a toggle, aiming at the switch rather than at the row.
    ///
    /// `XCUIElement.tap()` looked like the obvious call and silently did nothing, twice over. A
    /// SwiftUI `Toggle` publishes one accessibility element spanning the whole row, label and all,
    /// so the centre `tap()` aims at is over the descriptive text. And `isHittable` is true as
    /// soon as any sliver of that tall row is on screen, at which point its centre is underneath
    /// the tab bar. Neither miss fails: the tap lands on whatever is at those coordinates.
    ///
